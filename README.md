...
```shell
./gradlew test --tests="io.specmatic.inventory.ContractTest"
```
...
```shell
# Run contract tests
docker run --rm --network host -v "$(pwd):/usr/src/app" specmatic/specmatic test --port=8080
```
...
```shell
# Run contract tests
docker run --rm --network host -v "%cd%:/usr/src/app" specmatic/specmatic test --port=8080
```
